...
function centeredShortComplex(width = 1080, height = 1920) {
  return [
    `split=2[bg][fg]`,
    `[bg]scale=${width}:${height}:force_original_aspect_ratio=increase:flags=lanczos,crop=${width}:${height},boxblur=22:1[bg2]`,
    `[fg]scale=${width}:${height}:force_original_aspect_ratio=decrease:flags=lanczos[fg2]`,
    `[bg2][fg2]overlay=(W-w)/2:(H-h)/2:format=auto[centered]`,
    `[centered]fps=30,eq=saturation=1.12:contrast=1.045:brightness=0.01,unsharp=3:3:0.55:3:3:0.0[vout]`,
  ].join(";");
}
...
async function processUpscaleJob(job) {
  ...
  const vf = [
    ...
    "[bg2][fg2]overlay=(W-w)/2:(H-h)/2:format=auto[centered]",
    "[centered]eq=saturation=1.08:contrast=1.035:brightness=0.005,unsharp=5:5:0.85:5:5:0.0[vout]",
  ].join(";");
  run("ffmpeg", [
    "-y", "-i", src,
...
async function processJob(job) {
  ...
  for (const w of windows) {
    ...
    run("ffmpeg", [
      ...
      "-filter_complex", centeredShortComplex(1080, 1920), "-map", "[vout]", "-map", "0:a?",
